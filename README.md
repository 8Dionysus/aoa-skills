...
14. Read `docs/overlays/atm10/PROJECT_OVERLAY.md` or `docs/overlays/abyss/PROJECT_OVERLAY.md` for the current live family overlay packs.
...
- overlay preparation and thin downstream adaptation: `docs/OVERLAY_SPEC.md`, `docs/overlays/atm10/PROJECT_OVERLAY.md`, `docs/overlays/abyss/PROJECT_OVERLAY.md`, `generated/overlay_readiness.*`, `templates/PROJECT_OVERLAY.template.md`, `templates/PROJECT_OVERLAY_SKILL.template.md`
...
`generated/release_manifest.json` is the machine-readable portable release contract for the generated export stack. It pins artifact groups, digests, bundle revisions, and install-profile revisions, but it still does not replace repo-level release identity in `CHANGELOG.md`, the Git tag, or the GitHub release body.
...
This repository now has a mixed-status public core of 19 skills with first support artifacts, pinned bridge manifests, local validation for bundle shape and policy coherence, and source-owned section surfaces for bounded expand-time reads.
...
The current focus is keeping cross-family overlay parity stable while moving toward packaging prep through derived maintenance surfaces such as `generated/governance_backlog.*`, `generated/overlay_readiness.*`, `generated/skill_bundle_index.*`, and `generated/skill_graph.*`.
