...
Use this checklist when reviewing `titan-appserver-plan` output inside an explicit Titan service-cohort route.
...
- [ ] The plan stops before starting the server, child agents, or hidden runtime work.
- [ ] Launch commands, state paths, receipt paths, approval paths, and validation are named.
- [ ] Operator confirmations required before runtime execution are explicit.
