...
Use this checklist when reviewing `titan-closeout` output inside an explicit Titan service-cohort route.
...
- [ ] The closeout separates role, risk, provenance, gate, validation, and next-owner summary.
- [ ] Forge and Delta activity is reported only when receipt or ledger evidence exists.
- [ ] Candidate memory and replay results are not promoted during closeout.
