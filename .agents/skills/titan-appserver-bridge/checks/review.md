...
Use this checklist when reviewing `titan-appserver-bridge` output inside an explicit Titan service-cohort route.
...
- [ ] The bridge state is derived from explicit thread, turn, event, approval, or receipt refs.
- [ ] JSON-RPC or relay output does not start hidden child-agent execution.
- [ ] Bridge metrics are treated as derived status, not proof or memory truth.
