...
Use this checklist when reviewing `titan-thread-turn-binding` output inside an explicit Titan service-cohort route.
...
- [ ] Thread id, turn id, event refs, approval refs, and receipt refs are named.
- [ ] Missing, duplicate, or out-of-order continuity evidence is reported.
- [ ] Derived continuity state stays subordinate to bridge and receipt sources.
