...
| `live-harness/dispatch` | source locks, cohort expansion, safe adapters, host gates, private/public separation, and distinct native dispatch, hierarchy report, child trajectory, fixture execution, procedure-disposition, and objective-outcome posture without model calls. | `evals/runners/run_live_skill_dispatch.py` and its plan/schemas | `source` | Repair the deterministic harness contract before spending any live turn. |
