...
| `live-harness/dispatch` | source locks, cohort expansion, safe adapters, host gates, private/public separation, and paired outcome projection without model calls. | `evals/runners/run_live_skill_dispatch.py` and its plan/schemas | `source` | Repair the deterministic harness contract before spending any live turn. |
