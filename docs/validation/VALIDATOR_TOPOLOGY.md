...
| Evidence-readout completeness | Exact current-skill membership and `status`/`scope` parity in committed JSON, plus skill/status parity in Markdown quality and promotion projections. | `scripts/validation/validators/skill_evidence_readout_surface.py` | Regenerate both reports through their owner scripts; audit live usage and technique evidence separately. |
| Skill-effectiveness family review | One authored disposition per current skill, source path/status/scope parity, conditional lifecycle fields, public episode refs, and candidate-evidence authority limits. | `scripts/validation/validators/skill_effectiveness_family_review_surface.py` | Correct the current review record or its Method-Growth schema; route actual status changes through per-skill promotion review. |
| OS Abyss artifact bundle | `generated/release_manifest.json` as a release-contract subject for the root OS Abyss artifact verifier, durable evidence registry, materialized subject store, and trust-gate/latest consumer admission. | `scripts/validation/validate_abyss_machine_artifact_bundle.py`, `mechanics/release-support/manifests/release_manifest.bundle.json` | Fix the generated release manifest, bundle manifest, runner-provided `abyss-machine` verifier, durable registry lifecycle/source/trust-root metadata, subject-store materialization, or `abyss-machine` artifact policy class; rerun the release lane. |
...
The evidence-readout completeness gate proves only current JSON membership and
`status`/`scope` parity plus the matching Markdown skill/status projection. It
does not make live session counts, dispatch events, hook observations, reality
trials, or technique-drift evidence reproducible.
