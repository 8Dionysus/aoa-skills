...
from build_agon_epistemic_capability_candidates import (  # noqa: E402
    BuildError,
    ITEM_KEY,
    OUT,
    REGISTRY_ID,
...
REQUIRED_FIELDS = ['bounded_workflow_for', 'candidate_outputs', 'forbidden_effects', 'live_protocol', 'owner_repo', 'runtime_effect', 'capability_id', 'status', 'trigger_boundary']
...
ITEM_SCHEMA = (
    ...
    / "schemas"
    / "agon-epistemic-capability-candidate.schema.json"
)
REGISTRY_SCHEMA = (
    ROOT
    / "mechanics"
    ...
    / "epistemic-candidate-boundary"
    / "schemas"
    / "agon-epistemic-capability-candidate-registry.schema.json"
)
...
def validate_item(item: Any, item_validator: jsonschema.Draft202012Validator) -> str | None:
    ...
    if item.get("owner_repo") != "aoa-skills":
        return f"{key} owner_repo must be aoa-skills"
