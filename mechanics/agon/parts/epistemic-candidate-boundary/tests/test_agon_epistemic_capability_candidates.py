...
class AgonEpistemicSkillCandidateTests(unittest.TestCase):
    def test_generated_registry_shape(self) -> None:
        reg = json.loads(
            (ROOT / "generated/agon_epistemic_capability_candidates.min.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        self.assertEqual(len(reg["capabilities"]), 9)
        ...
        self.assertEqual(
            subprocess.run(
                [
                    ...
                    str(
                        ROOT
                        / "mechanics/agon/parts/epistemic-candidate-boundary/scripts/build_agon_epistemic_capability_candidates.py"
                    ),
                    "--check",
                ],
                cwd=ROOT,
        ...
        self.assertEqual(
            subprocess.run(
                [
                    ...
                    str(
                        ROOT
                        / "mechanics/agon/parts/epistemic-candidate-boundary/scripts/validate_agon_epistemic_capability_candidates.py"
                    ),
                ],
                cwd=ROOT,
            ).returncode,
    ...
    def test_registry_schema_validates_without_ref_workaround(self) -> None:
        schema = json.loads(
            (
                ROOT
                / "mechanics/agon/parts/epistemic-candidate-boundary/schemas/agon-epistemic-capability-candidate-registry.schema.json"
