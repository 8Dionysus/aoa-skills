...
CONFIG_PATH = (
    ...
    / "config"
    / "agon_capability_binding_candidates.seed.json"
)
GENERATED_PATH = ROOT / "generated" / "agon_capability_binding_candidates.min.json"
EXPECTED_SCHEMA_VERSION = "agon-capability-binding-candidates.seed/0.1"
EXPECTED_INDEX_SCHEMA = "agon-capability-binding-candidates-index/0.1"
EXPECTED_BRIDGE_KIND = "bounded_workflow_candidate"
EXPECTED_TOTAL = 14
EXPECTED_REPO = "aoa-skills"
EXPECTED_SOURCE_OWNER_BINDING = "Agents-of-Abyss/generated/agon_move_owner_binding_registry.min.json"
...
def validate_config(config: dict[str, Any]) -> None:
    ...
    for candidate in candidates:
        ...
        if candidate.get("source_owner_binding") != EXPECTED_SOURCE_OWNER_BINDING:
            raise ValidationError(
                f"{cid}: source_owner_binding must be {EXPECTED_SOURCE_OWNER_BINDING}"
            )
...
def build_index(config: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "wave": "IV",
        "title": "Agon capability binding candidate index",
        ...
        "stop_line": (
            "candidate requests do not create promoted methods, promoted callable skills, live arena protocol, "
