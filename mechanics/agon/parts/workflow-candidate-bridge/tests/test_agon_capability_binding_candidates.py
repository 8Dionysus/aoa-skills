...
def load_builder():
    path = (
        ...
        / "workflow-candidate-bridge"
        / "scripts"
        / "build_agon_capability_binding_candidates.py"
    )
    spec = importlib.util.spec_from_file_location("agon_capability_binding_builder_test", path)
...
def test_agon_capability_binding_candidates_current() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "mechanics/agon/parts/workflow-candidate-bridge/scripts/build_agon_capability_binding_candidates.py",
            "--check",
        ],
        cwd=ROOT,
        text=True,
...
def test_agon_capability_binding_candidate_shape() -> None:
    data = json.loads((ROOT / "generated" / "agon_capability_binding_candidates.min.json").read_text())
    assert data["wave"] == "IV"
