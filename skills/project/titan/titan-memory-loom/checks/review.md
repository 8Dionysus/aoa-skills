...
Use this checklist when reviewing `titan-memory-loom` output inside an explicit Titan service-cohort route.
...
- [ ] Every memory record has source refs, confidence, state, and authority note.
- [ ] Records remain candidate unless an owner surface has promoted them.
- [ ] Retention, redaction, or tombstone risk is named when visible.
