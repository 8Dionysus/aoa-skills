...
Use this checklist when reviewing `titan-mutation-gate` output inside an explicit Titan service-cohort route.
...
- [ ] Forge mutation intent, target paths, validation, and rollback or stop posture are explicit.
- [ ] Atlas, Sentinel, and Mneme precheck evidence is present or the gate fails closed.
- [ ] The output returns allowed, blocked, or pending state with receipt or ledger refs.
