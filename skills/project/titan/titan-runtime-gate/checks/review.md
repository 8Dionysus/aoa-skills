...
Use this checklist when reviewing `titan-runtime-gate` output inside an explicit Titan service-cohort route.
...
- [ ] The gate kind, actor, target, approval, and receipt are all matched.
- [ ] Forge mutation and Delta judgment cannot be opened by the wrong approval.
- [ ] Ambiguity fails closed with a next owner-visible step.
