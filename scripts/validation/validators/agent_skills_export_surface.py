...
from runtime.skill_runtime_guardrails import (
...
def validate_export_skill_sets(
    ...
    agent_skills_local_adapter_surface.validate_local_adapter_skill_sets(
        manifest_by_name=indexes.manifest_by_name,
        manifest_min_by_name=indexes.manifest_min_by_name,
...
def validate_exported_skill_directory(
    ...
    catalog_result = agent_skills_catalog_surface.validate_export_catalog_entries(
        repo_root=repo_root,
        skill_name=skill_dir.name,
        skill_md=skill_md,
        openai_yaml=openai_yaml,
        source_by_name=indexes.source_by_name,
        agent_by_name=indexes.agent_by_name,
        export_by_name=indexes.export_by_name,
        manifest_by_name=indexes.manifest_by_name,
        runtime_by_name=indexes.runtime_by_name,
    ...
    validate_exported_resource_surfaces(
        ...
        skill_dir,
        indexes,
        catalog_result.agent_entry,
        catalog_result.export_entry,
        catalog_result.manifest_entry,
        errors,
    )
    ...
    validate_exported_description_router_surfaces(
        ...
        skill_dir,
        description,
        catalog_result.source_scope,
        catalog_result.source_invocation_mode,
        allow_implicit,
        activation_policy,
        router_entry,
        errors,
...
def validate_project_core_kernel_surfaces(
    ...
    agent_skills_project_surface.validate_project_core_kernel_surfaces(
        repo_root, skills_root, docs, indexes, errors
    )


def validate_project_core_outer_ring_surfaces(
    docs: ExportSurfaceDocuments, indexes: ExportSurfaceIndexes, errors: list[str]
) -> None:
    agent_skills_project_surface.validate_project_core_outer_ring_surfaces(docs, indexes, errors)


def validate_project_risk_ring_surfaces(
    docs: ExportSurfaceDocuments, indexes: ExportSurfaceIndexes, actual_names: set[str], errors: list[str]
) -> None:
    agent_skills_project_surface.validate_project_risk_ring_surfaces(docs, indexes, actual_names, errors)


def validate_project_foundation_profile(docs: ExportSurfaceDocuments, errors: list[str]) -> None:
    agent_skills_project_surface.validate_project_foundation_profile(docs, errors)
...
def validate(repo_root: pathlib.Path) -> ValidationResult:
    ...
    agent_skills_project_surface.validate_project_risk_ring_surfaces(
        docs, indexes, actual_names, errors
    )
