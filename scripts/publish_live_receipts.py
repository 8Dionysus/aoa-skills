...
DEFAULT_LOG_PATH = REPO_ROOT / ".aoa" / "live_receipts" / "session-harvest-family.jsonl"
ALLOWED_EVENT_KINDS = {
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Append bounded session-harvest-family receipts to the owner-local live JSONL log."
    )
    parser.add_argument(
        "--input",
        action="append",
    ...
    parser.add_argument(
        ...
        help="Owner-local JSONL log that should receive newly published receipts.",
    )
...
def validate_receipt(receipt: dict[str, Any], *, location: str) -> None:
    ...
    if event_kind not in ALLOWED_EVENT_KINDS:
        raise ReceiptPublishError(
            f"{location}.event_kind: unsupported session receipt kind {event_kind!r}"
        )
