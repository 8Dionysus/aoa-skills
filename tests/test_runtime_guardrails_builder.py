...
class RuntimeGuardrailsBuilderTests(unittest.TestCase):
    ...
    def test_generated_guardrail_artifacts_exist(self):
        trust_gate = json.loads(
            (REPO_ROOT / "generated" / "repo_trust_gate_manifest.json").read_text()
        )
        allowlist = json.loads(
            (REPO_ROOT / "generated" / "permission_allowlist_manifest.json").read_text()
        )
        context_guard = json.loads(
            (REPO_ROOT / "generated" / "skill_context_guard_manifest.json").read_text()
        )
        tool_schemas = json.loads(
            (
                REPO_ROOT / "generated" / "runtime_guardrail_tool_schemas.json"
            ).read_text()
        )

        self.assertEqual(
            trust_gate["profile"], "codex-facing-wave-6-runtime-guardrails"
        )
