...
class ReportSkillEvaluationTests(unittest.TestCase):
    def make_repo(self, *, include_snapshots: bool = True) -> Path:
        repo_root = Path(tempfile.mkdtemp(prefix="aoa-skills-eval-report-"))
        ...
        (review_dir / "aoa-change-protocol.md").write_text(
    ...
    def run_main(self, repo_root: Path, argv: list[str]) -> tuple[int, str, str]:
        ...
        with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
            exit_code = report_skill_evaluation.main(argv, repo_root=repo_root)
    ...
    def test_text_report_succeeds(self) -> None:
        ...
        self.assertIn("# Skill evaluation report", stdout)
        self.assertIn("## aoa-change-protocol", stdout)
        self.assertIn("use_snapshots=1", stdout)
        self.assertIn("canonical eval ready: true", stdout)
    ...
    def test_json_report_is_deterministic(self) -> None:
        ...
        exit_code, stdout, stderr = self.run_main(
            repo_root,
            ["--skill", "aoa-change-protocol", "--format", "json"],
        )
        ...
        self.assertEqual("aoa-change-protocol", payload["skills"][0]["name"])
        self.assertEqual(1, payload["skills"][0]["use_snapshot_count"])
        self.assertTrue(payload["skills"][0]["canonical_eval_ready"])
    ...
    def test_fail_on_canonical_gaps_returns_exit_1(self) -> None:
        repo_root = self.make_repo(include_snapshots=False)
        ...
        self.assertEqual("", stderr)
        self.assertIn("canonical eval ready: false", stdout)
